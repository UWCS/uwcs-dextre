...
def make_user_site_config(username):
    ...
    config = render_to_string(
        "accounts/members_template.conf",
        {
            "user": username,
    ...
    config_file = open(
        "{sites_available}/members-{nickname}.conf".format(
            sites_available=settings.APACHE_SITES_AVAILABLE, nickname=username
        ),
        "w",
    )
    ...
    os.symlink(
        ...
        "{sites_enabled}/members-{nickname}.conf".format(
            sites_enabled=settings.APACHE_SITES_ENABLED, nickname=username
        ),
    )
...
def make_user_site_placeholder(username, uid):
    ...
    html = render_to_string("accounts/usersite_template.html", {"nickname": username})

    # Write the file to disk
    index_path = "{website_dir}/{nickname}/index.html".format(
        website_dir=settings.APACHE_WEBSITE_DIR, nickname=username
    )
...
def create_ldap_user(account_id):
    ...
    search_string_user = "uid={username},ou=People,dc=uwcs,dc=co,dc=uk".format(
